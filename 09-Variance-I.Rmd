...
<!-- Here, we need also to look at Type II versus type III + packages effectsize and afex. Discuss about eta^2, omega^2, Cohen's f etc (effect size in percent of variance) + the absolute effect. Also a power analysis with the pwr package and a way to estimate best n given a goal. Finally, one or two Shiny apps that allow to play with partition of variance to feel it better. A supplemental section that presents alternate homoscedasticity tests (Levene, ...). For effect size: https://cran.r-project.org/web/packages/effectsize/vignettes/anovaES.html -->

<!-- Also add a facultative section on FDR as an alternative when there are many simultaneous tests -->

##### Objectifs {.unnumbered}

-   Pouvoir comparer les moyennes de plus de deux populations simultanément en utilisant des techniques de décomposition de la variance
...
## Le danger des tests multiples {#testsmultiples}

Avant toute chose, nous préparons notre session R. Nous avons besoin de `SciViews::R` bien entendu, mais aussi des extensions `"infer"` et `"model"`. Tout cela est chargé en mémoire comme suit (n'oubliez pas ces codes dans un "chunk" de "setup" en début de vos documents Quarto ou R Markdown) :

```{r anova1fact, echo=FALSE}
#'
...
Les tests *t* de Student et de Wilcoxon-Mann-Whitney sont limités à la comparaison de deux populations. Poursuivons notre analyse des crabes *L. variegatus*. Rappelez-vous, nous avons deux variétés (variable `species`, `B` pour bleue et `O` pour orange). Si nous voulons comparer simultanément les mâles et les femelles des deux variétés, cela nous fait quatre sous-populations à comparer (nous utilisons ici la fonction `paste()` qui rassemble des chaînes de caractère avec le tiret comme caractère séparateur `sep = "-"` pour former une variable facteur à quatre niveaux, `B-F`, `B-M`, `O-F`, `O-M`). Nous transformons cette variable en facteur à l'aide de `factor()` et nous ajoutons un label avec `labelise()`.
...
La Fig. \@ref(fig:crabs-rear) montre la largeur à l'arrière de la carapace pour les quatre groupes ainsi individualisés. Une représentation graphique adéquate avant de réaliser notre analyse lorsque le nombre de réplicats est important est le graphique en violon sur lequel nous superposons au moins les moyennes, et de préférence, les points également. Si le nombre de réplicats est plus faible, mais toujours supérieur à 7-8, nous pourrions utiliser le même type de graphique, mais avec des boites à moustaches plutôt (voir plus loin, Fig. \@ref(fig:anova2)). Avec encore moins de réplicats nous présenterions les points et les moyennes uniquement.

```{r crabs-rear, fig.cap="Largeur arrière en fonction du groupe de crabes *L. variegatus*. Graphique adéquat pour comparer les moyennes et distributions dans le cas d'un nombre important de réplicats (moyennes en rouge + observations individuelles en noir semi-transparent superposées à des graphiques en violon)."}
...
`r h5p(228, "A09Hb_balanced", height = 270, toc = "Plan balancé d'un expérience")`
...
`r h5p(229, "A09Hc_multiple", height = 270, toc = "Comparaison deux à deux")`

## ANOVA à un facteur {#anovaunfacteur}
...
Cette variance peut être *partitionnée*. C'est-à-dire que, si la variance totale se mesure d'un point A à un point C, on peut mesurer la part de variance d'un point A à un point B, puis d'autre part, d'un point B à un point C, et dans ce cas,
...
`r h5p(100, "A09Hd_anova1", height = 400, toc = "Hypothèse nulle de l'ANOVA")`
...
```{r anova1, echo=FALSE, fig.cap="Décomposition de la variance dans un cas à trois populations A, B et C fictives.", purl=FALSE}
...
chart(data = dat, Réponse ~ Population %col=% Population) +
  ...
  geom_segment(data = annot2, aes(x = xstart, xend = xend, y = y , yend = y),
    color = annot2$color, linetype = "dotted", linewidth = 0.5) +
...
Notons que ce modèle à trois termes représente bien la situation qui nous intéresse, mais aussi, qu'il décompose la variance totale (entre $\mu$ et chaque point observé) en deux composantes : ce que nous appellerons le terme **inter** représentant l'écart entre la moyenne globale $\mu$ et la moyenne de la sous-population concernée ($\tau_j$) et le terme **intra** depuis cette moyenne de la sous-population jusqu'au point observé ($\epsilon_i$).

##### À vous de jouer ! {.unnumbered}

`r h5p(99, "A09He_gof", height = 400, toc = "Variance intergroupe dans l'ANOVA")`
...
`r h5p(101, "A09Hf_ddl", height = 400, toc = "Analyse du tableau de l'ANOVA")`

### Distribution *F*

La distribution *F* est une distribution asymétrique n'admettant que des valeurs nulles ou positives, d'une allure assez similaire à la distribution du $\chi^2$ que nous avons étudié au module \@ref(districhi2). Elle est appelée loi de Fisher, ou encore, loi de Fisher-Snedecor. Elle a une asymptote horizontale à $+\infty$. La distribution *F* admet deux paramètres, respectivement les degrés de liberté au numérateur (inter) et au dénominateur (intra). La Fig. \@ref(fig:fplot) représente la densité de probabilité d'une loi *F* typique[^09-variance-i-3].

[^09-variance-i-3]: Les fonctions qui permettent les calculs relatifs à la distribution *F* dans R sont `<q|p|r|d>f()`. En outre, `dist_f()` génère un objet permettant d'en tracer facilement le graphique. Leur utilisation est similaire à celle des distributions vues aux modules précédents.

<!-- et les snippets correspondants dans la SciViews Box sont disponibles à partir de `.if`. -->

```{r fplot, echo=FALSE, warning=FALSE, fig.cap = "Allure typique de la densité de probabilité de la distribution *F* (ici ddl inter = 5 et ddl intra = 20). Plus *F~obs* est grand, plus l'hypothèse nulle est suspecte. La zone de rejet est donc positionnée à droite (en rouge).", purl=FALSE}
...
```{r resid1, echo=FALSE, results='hide', fig.cap="Graphique quantile-quantile appliqué aux résidus d'une ANOVA pour déterminer si leur distribution se rapproche d'un loi normale.", purl=FALSE}
...
`r h5p(102, "A09Hg_variance", height = 400, toc = "Variance intragroupe dans l'ANOVA")`

##### Résolution de notre exemple {.unnumbered}

Nous commençons par déterminer si nous avons une homoscédasticité. Considérons un seuil $\alpha$ de 5% pour tous nos tests. Ensuite, nous utilisons la fonction£


`bartlett.test()` avec une formule qui représente bien la variable réponse *en fonction de* (le tilde `~`) la variable explicative, soit ici, `aspect ~ group` :
...
Ici cela ne fonctionne pas. La transformation inverse (`exp()`) peut être essayée, mais ne stabilise pas suffisamment la variance. Après divers essais, il s'avère qu'une transformation puissance cinq stabilise bien la variance.
...
Nous observons dans le tableau de l'ANOVA que la valeur *p* est très faible et inférieure à $\alpha$. Nous rejetons $H_0$. Nous pouvons dire que le ratio largeur arrière / max à la puissance cinq diffère significativement entre les groupes (ANOVA, F = 151, ddl = 3 & 196, valeur *p* \<\< 10^-3^).
...
`r h5p(230, "A09Hh_posthoc", height = 400, toc = "tests post hoc")`
...
`r learnr("A09La_anova", title = "ANOVA et tests post-hoc", toc = "Analyse de variance et tests post-hoc")`

```{r assign_A09Ia_acidification, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("A09Ia_acidification", part = NULL,
    url = "https://github.com/BioDataScience-Course/A09Ia_acidification",
    course.ids = c(
      'S-BIOG-027' = !"A09Ia-{YY}M-acidification"),
    course.urls = c(
      'S-BIOG-027' = !"{assign_url$A09Ia_acidification}"),
    course.starts = c(
      'S-BIOG-027' = !"{class1_start(mod, 'A09')}"),
    course.ends = c(
      'S-BIOG-027' = !"{n3_end(mod, 'A09')}"),
    term = "Q2", level = 3,
...
```{r kruskalwallis, echo=FALSE}
#'
#' ### Test de Kruskal-Wallis {#kruskalwallis}
#'
```

Les conditions d'application de l'ANOVA sont assez restrictives, en particulier l'homoscédasticité et le normalité des résidus. Dans notre exemple, nous avons pu stabiliser la variance par une transformation puissance cinq, mais la distribution des résidus n'est pas optimale. Nous pouvons tout aussi bien décider de nous orienter vers la version non paramétrique équivalente : le **test de Kruskal-Wallis**.
...
-   variable dite **réponse** quantitative,
-   une variable dite **explicative** qualitative à trois niveaux ou plus,
-   les distributions au sein des différentes sous-populations sont, si possible, similaires, mais de forme quelconque.

### Test "post hoc" non paramétrique

Nous devrions également réaliser des tests "post hoc" lorsque nous rejetons $H_0$ du Kruskal-Wallis. Les versions non paramétriques des tests de comparaisons multiples sont moins connues. Nous pourrions effectuer des comparaisons deux à deux avec des tests de Wilcoxon-Mann-Whitney en appliquant une correction de Bonferroni.

Une autre option consiste à utiliser une version non paramétrique du test de Tukey basée sur les rangs, dit test post hoc non paramétrique asymptotique. Les explications assez techniques sont accessibles [ici](https://onlinelibrary.wiley.com/doi/abs/10.1002/1521-4036%28200109%2943%3A5%3C553%3A%3AAID-BIMJ553%3E3.0.CO%3B2-N). Ce test est disponible dans le package R {nparcomp}.
...
## Récapitulatif des exercices {#recapitulatifa09}

Vous venez de terminer le module 9. Ce module vous a permis d'apprendre une grande partie des subtilités qui se cachent derrière l'analyse de variance. Pour évaluer votre compréhension de cette matière, vous aviez les exercices suivants à réaliser :
...
-   [Podcast sections 9.1 - 9.4 (NotebookLM) - téléchargement](https://filedn.com/lzGVgfOGxb6mHFQcRn9ueUb/sdd-umons/sdd1_09/A09_91-4podcast_notebooklm.m4a)
-   [Présentation vidéo sections 9.1 - 9.4 (NotebookLM)](https://filedn.com/lzGVgfOGxb6mHFQcRn9ueUb/sdd-umons/sdd1_09/A09_91-4video_notebooklm.mp4)
-   [Slides sections 9.1 - 9.4 (NotebookLM)](https://filedn.com/lzGVgfOGxb6mHFQcRn9ueUb/sdd-umons/sdd1_09/A09_91-4slides_notebooklm.pdf)
-   [Infographics sections 9.1 - 9.4 partiel (NotebookLM)](https://filedn.com/lzGVgfOGxb6mHFQcRn9ueUb/sdd-umons/sdd1_09/A09_91-4infographics_notebooklm.png)
-   [Quizz sections 9.1 - 9.4 (NotebookLM)](https://filedn.com/lzGVgfOGxb6mHFQcRn9ueUb/sdd-umons/sdd1_09/A09_91-4quiz_notebooklm.html)

`r h5p(273, "A09Za_flash91-4", height = 400, toc = "Flashcards sections 9.1 - 9.4")`


##### Code des exemples {.unnumbered}

-  [Document HTML](more/09-Variance-I.html)
-  [Script R](https://github.com/BioDataScience-Course/sdd-umons-2025/blob/main/R/09-Variance-I.R){target="top"}
