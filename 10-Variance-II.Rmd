...
## ANOVA à deux facteurs {#anovadeuxfacteurs}
...
`r h5p(117, "A10Hb_variable_quali", height = 400, toc = "Différents types de variables")`
...
La Fig. \@ref(fig:crabs2boxplot2) est une version améliorée avec les observations et les moyennes pour chaque sous-groupe ajoutées au graphique selon la même technique que nous avions utilisée pour l'ANOVA à un facteur.
...
`r h5p(118, "A10Hc_bartlett", height = 400, toc = "Homoscédasticité et hétéroscédasticité")`
...
Nous interprétons chacun des deux tests séparément. Dans notre cas, nous pouvons dire avec un seuil $\alpha$ de 5% que nous rejetons $H_0$ dans les deux cas (*p* \< $\alpha$). Donc le rapport largeur arrière sur largeur max de la carapace (élevé à la puissance 5) est significativement différent au seuil $\alpha$ de 5% à la fois en fonction de l'espèce (*F* = 17,2, ddl = 1 et 197, valeur *p* \<\< 0,001) et du sexe (*F* = 436, ddl = 1 et 197, valeur *p* \<\< 0,001)[^10-variance-ii-1].

[^10-variance-ii-1]: Notez que si vous incluez le tableau de l'ANOVA dans votre rapport ou dans une publication, il n'est pas nécessaire de répéter les résultats des tests entre parenthèses. Vous pouvez juste vous référer au tableau en question.
...
## Modèle croisé complet {#anovacomplete}

```{r anova2complete, echo=FALSE}
#'
#' ### ANOVA à deux facteurs croisés complet {#anova2complete}
#'
```

Le modèle ANOVA que nous venons de faire s'appelle un **modèle croisé** parce que les mesures sont effectuées pour chaque combinaison des niveaux des deux variables facteurs explicatives, et ce, de manière indépendante (les observations d'un niveau ne sont pas dépendantes de celles d'un autre niveau)[^10-variance-ii-2].

[^10-variance-ii-2]: De plus, nous avons ici un **plan balancé** puisque le nombre de réplicats pour chaque niveau est le même. C'est une situation optimale qu'il faut toujours chercher à atteindre pour une ANOVA, même si un nombre différent d'observations par niveau est également accepté.
...
`r h5p(113, "A10Hd_modele", height = 400, toc = "Modèle croisé complet")`
...
`r h5p(112, "A10He_interactions", height = 400, toc = "Graphique des interactions")`

`r learnr("A10La_anova2", title = "ANOVA à 2 facteurs", toc = "Analyse de variance à 2 facteurs")`
...
```{r assign_A10Ia_anova2_I, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("A10Ia_anova2", part = "I",
    url = "https://github.com/BioDataScience-Course/A10Ia_anova2",
    course.ids = c(
      'S-BIOG-027' = !"A10Ia-{YY}M-anova2"),
    course.urls = c(
      'S-BIOG-027' = !"{assign_url$A10Ia_anova2}"),
    course.starts = c(
      'S-BIOG-027' = !"{class1_start(mod, 'A10')}"),
    course.ends = c(
      'S-BIOG-027' = !"{n3_end(mod, 'A10')}"),
    term = "Q2", level = 3,
    toc = "ANOVA 2 facteurs, part I")
```

## Facteurs hiérarchisés {#anovahierarchisee}

```{r anova2hierar, echo=FALSE}
#'
...
Voici un exemple concret. Un gros échantillon homogène d'œufs déshydratés est réparti entre six laboratoires différents en vue de la détermination de la teneur en matières grasses dans cet échantillon. Le but de la manœuvre est de déterminer si les laboratoires donnent des résultats consistants entre eux. Les deux techniciens de chaque laboratoire sont libellés `one` et `two`, mais ce sont en fait à chaque fois des techniciens *différents* dans chaque laboratoire[^10-variance-ii-3].

[^10-variance-ii-3]: La variable `Sample` valant `G` ou `H` ne sera pas utilisée ici. En fait, au départ, les initiateurs de l'expérience ont fait croire aux laboratoires qu'il s'agissait de deux échantillons différents alors que c'est le même en réalité.
...
## Effet aléatoire {#effetsaleatoires}

```{r anova2splitplot, echo=FALSE}
#'
...
-   Soit, nous échantillonnons aléatoirement dans la population un nombre restreint de modalités, et nous restons intéressés par tous les cas possibles. À ce moment-là, la variable facteur doit être traitée comme **aléatoire**.
...
Prenons un exemple concret. Dans une analyse réalisée en 1972 par Davies et Goldsmith, six lots différents de pénicilline provenant d'une production de ce médicament sont comparés afin d'estimer la variabilité de l'efficacité antibiotique en fonction du lot de production. La mesure consiste à incuber une bactérie *Bacillus subtilis* dans une série de boites de pétri (variable `plate`). Dans chaque boite, un volume de solution des six lots de pénicilline (variable `sample`) est ajouté. Les six ajouts sont réalisés à espacement régulier, la boite de pétri étant divisée en six "parts de tartes" égales). L'effet de l'antibiotique est mesuré au travers du diamètre de la zone de non-croissance de la bactérie autour du point d'injection (variable `diameter`).
...
Étant donné que chacun des six lots d'antibiotiques est testé dans chaque boite de pétri, nous serions tentés de penser que nous avons un plan croisé classique. La fonction `replications()` permet de déterminer rapidement le nombre de réplicats dans un plan d'expérience. Il s'utilise avec les mêmes données et la même formule que celle qu'on utiliserait pour faire l'ANOVA. Ici, nos données sont balancées, donc idéales avec 24 réplicats pour chaque lot (`sample`) et six mesures par boite de pétri (`plate`), mais nous n'avons qu'une seule mesure par lot et par plaque. Cela nous empêche d'étudier les interactions éventuelles (si les boites de pétri sont produites de manière identique et placée au hasard dans un incubateur fournissant un environnement homogène, il y a peu de chances qu'il y ait de telles interactions).
...
Les deux facteurs sont significatifs au seuil $\alpha$ de 5%. Pour finir, une analyse des résidus confirme que les conditions d'application de l'ANOVA sont rencontrées (les écarts des points à la droite de référence restent dans le domaine de l'acceptable pour ce graphique quantile-quantile des résidus du modèle) :
...
Un des modèles pour lequel les valeurs *p* sont calculables est le modèle dit en [parcelles divisées](https://stat.ethz.ch/education/semesters/as2015/anova/08_Split_Plots.pdf) (*split-plot* en anglais). Cette situation apparaît lorsque, parmi deux facteurs, l'un est plus difficile à diviser en sous-unités *indépendantes*. C'est le cas ici avec nos boites de pétri qui forment chacune un petit microcosme unique. Il est possible de diviser la boite de pétri en sous-régions. C'est ce qui a été fait ici pour tester les six lots, mais ces sous-régions ne **sont pas indépendantes** les unes des autres, elles continuent d'être liées entre elles au sein de la même boite de pétri. Nous pouvons néanmoins considérer la boite de pétri comme facteur aléatoire dans ce modèle split-plot. Dans ce cas, nous avons intérêt à utiliser `lmeTest::lmer()` qui calculera ces valeurs *p* là où elles sont pertinentes[^10-variance-ii-4]. Dans la formule, un facteur aléatoire simple s'écrit `(1 | facteur)`. Il considère un décalage constant pour tous les autres facteurs qui est propre à chaque niveau du facteur aléatoire. Concrètement ici, une boite de pétri donne des résultats globalement meilleurs ou moins bons pour l'ensemble des six lots de pénicilline. Cela donne :

[^10-variance-ii-4]: Notez toutefois qu'il est conseillé de manière générale de **ne pas** utiliser les valeurs *p* du tout, mais de privilégier d'autres approches comme le rapport de la log-vraisemblance entre modèles, voir [ici](https://onlinelibrary.wiley.com/doi/abs/10.1111/j.1467-9450.2010.00852.x), mais cela sort du cadre de ce cours.
...
Le travail d'interprétation du facteur aléatoire `plate` est terminé, mais il nous reste à présent à réaliser un test post-hoc de comparaisons multiples sur le facteur fixe `sample` qui est significatif au seuil $\alpha$ de 5% (voir tableau de l'ANOVA plus haut). Cela se réalise de la même façon qu'avec l'ANOVA classique :
...
N'oublions pas de réaliser une analyse des résidus de ce modèle pour vérifier que ces derniers ont bien une distribution normale et que les variances sont homogènes (homoscédasticité). Malheureusement, les graphiques `chart()` ne sont pas encore implémentés pour les objets **lmerXXX** et nous devons les calculer "à la main".
...
Vous pouvez constater au travers de ces graphiques que les résidus sont les mêmes que pour le modèle fixe. Nous concluons donc, encore une fois, que la distribution des résidus est compatible avec le modèle.
...
Inutile d'utiliser `anova()` ici puisqu'il n'y a plus aucun facteur fixe. Nous interprétons les deux facteurs à l'aide de leurs écarts types respectifs calculés comme 0,85 toujours pour `plate` et 1,93 pour `sample`. Calculons les intervalles de confiance sur ces écarts types.
...
```{r assign_A10Ia_anova2_II, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("A10Ia_anova2", part = "II",
    url = "https://github.com/BioDataScience-Course/A10Ia_anova2",
    course.ids = c(
      'S-BIOG-027' = !"A10Ia-{YY}M-anova2"),
    course.urls = c(
...
En lisant dans le sous-tableau `Fixed effects:`, nous avons `a` = 251,4 et `b` = 10,5 dans la colonne `Estimate` en regard de `(Intercept)` et `days`, soit ordonnée à l'origine en anglais et pente relative à la variable `days`. Cela donne l'équation suivante : Réaction = 251,4 + 10,5 \* days. Le temps de réaction moyen au jour zéro est de 251 ms, et l'augmentation est de 10,5 ms par jour, toujours en moyenne (mais nous anticipons ici sur ce qu'on appelle un modèle linéaire que nous étudierons plus en détail au cours de SDD II l'an prochain).
...
`r learnr("A10Lb_anova2mixte", title = "ANOVA à 2 facteurs mixte", toc = "Analyse de variance à 2 facteurs mixte")`
...
-   Dans le cas où les conditions d'application de l'ANOVA à mesures répétées ne sont pas rencontrées, nous pouvons utiliser un test non paramétrique équivalent : le [**test de Friedman**](https://www.datanovia.com/en/fr/lessons/test-de-friedman-dans-r/).

## Syntaxe de R

```{r syntaxr, echo=FALSE}
#'
...
Dans cette dernière section du module 10, nous revenons sur nos outils logiciels. C'est bien de connaître les techniques de visualisation et d'analyse des données, mais c'est encore mieux de pouvoir les appliquer en pratique... et pour cela il faut maîtriser un ou plusieurs logiciels. Nous avons utilisé principalement **[R](https://www.r-project.org)**, avec l'interface proposée par RStudio. Nous allons maintenant approfondir quelque peu nos connaissances en R.

```

Vous avez pu vous rendre compte que R n'est pas qu'un logiciel de science des données. Il comprend également un **langage informatique** du même nom[^10-variance-ii-5] dédié à la manipulation, la visualisation et l'analyse des données.

[^10-variance-ii-5]: Historiquement, ce langage s'appelait **S** à l'origine dans les années 1970 alors qu'il a été inventé par John Chambers et ses collègues aux laboratoires Bell. R est une implémentation open source de S écrite dans les années 1990 par Ross Ihaka et Robert Gentleman. Ensuite R a pris de l'importance à tel point qu'on parle maintenant du langage R (sachant bien qu'il s'agit d'un *dialecte* du langage S).
...
`r h5p(115, "A10Hf_syntaxe", height = 400, toc = "Trois principales syntaxes")`
...
`r h5p(119, "A10Hg_indices", height = 400, toc = "Indiçage dans R")`
...
Attention : rappelez-vous ici que `group_by_()` et `summarise_()` ne sont pas compatibles avec `n()` (vous devez utilise `fn()` ou `fnobs()` selon que vous vouliez compter tous les cas ou seulement ceux hors valeurs manquantes).
...
`r h5p(114, "A10Hh_select", height = 400, toc = "Sélection de variables")`
...
Vous pouvez aussi utiliser la fonction "svTidy" `mutate_()` de `SciViews::R` bien sûr :
...
![Prolifération de standards par xkcd.](https://imgs.xkcd.com/comics/standards.png)

</center>

##### À vous de jouer ! {.unnumbered}

`r learnr("A10Lc_syntaxr", title = "Syntaxes dans R", toc = "Syntaxes dans R")`
...
```{r assign_A07Ga_human_IV, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment2"))
  assignment2("A07Ga_human", part = "IV",
...
Tout au long des modules 6 à 10, vous avez appris à réaliser des tests d'hypothèses avec R et RStudio. Il est maintenant temps de faire un bilan de vos capacités à écrire le code R pour reproduire de tels tests dans un **challenge de rapidité pour la réalisation de tests d'hypothèses.** Vous vous confronterez entre vous de manière individuelle pour déterminer celui ou celle qui arrivera à reproduire fidèlement le plus d'analyses possible.

##### À vous de jouer ! {.unnumbered}

```{r assign_A10Ca_infer, echo=FALSE, results='asis', purl=FALSE}
if (exists("challenge"))
  challenge("A10Ca_infer", part = NULL,
    url = "https://github.com/BioDataScience-Course/A10Ca_infer",
    course.ids = c(
      'S-BIOG-027' = !"A10Ca-{YY}M-infer"),
    course.urls = c(
      'S-BIOG-027' = !"{assign_url$A10Ca_infer}"),
    course.starts = c(
      'S-BIOG-027' = !"{mod['A03', 'challenge']} 10:00:00"),
    course.ends = c(
      'S-BIOG-027' = !"{mod['A03', 'challenge']} 13:00:00"),
    term = "Q2",
    toc = "Challenge tests d'hypothèses")
```
...
`r launch_shiny("https://sdd.umons.ac.be/A10Ca_infer/", height = 800, delay = 20, toc = NULL, alt1 = "*Cliquez pour visualiser le classement.*", alt2 = "*Cliquez pour visualiser le classement.*")`
:::


## Récapitulatif des exercices {#recapitulatifa10}
...
-  [Document HTML](more/10-Variance-II.html)
-  [Script R](https://github.com/BioDataScience-Course/sdd-umons-2025/blob/main/R/10-Variance-II.R){target="top"}
