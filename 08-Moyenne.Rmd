...
<!-- -We need to look at a way to estimate best n given a goal. Work on effect size and absolute effect in a way that it would be homogeneous with those of Chi^2 and ANOVA. We also need a nicer presentation of output in reportit + labels, incl. in French. A Shiny app to feel the various aspects presented here. Also a supplemental section that presents bootstrap and bootstrapped t test + a Bayeasian alternative. -->
...
-   Appréhender l'intervalle de confiance, être capable de le calculer et de l'utiliser
...
-   Connaître également le test de Wilcoxon-Mann-Withney, et pouvoir déterminer quand l'utiliser à la place du test *t* de Student

##### Prérequis {.unnumbered}

Ce module est élaboré sur les notions de probabilité, de loi de distribution statistique et de test d'hypothèse abordées dans les deux modules précédents \@ref(probacorr) et \@ref(districhi2). La matière de ces deux modules doit donc être maîtrisée avant d'aller plus avant ici.

## Une histoire de bière... {#histoiredebiere}
...
Pour la petite histoire, Gosset a travaillé pour une certaine brasserie irlandaise du nom de Guinness au début du 20^e^ siècle. C'est en étudiant la variabilité de sa bière d'un cru à l'autre que Gosset a découvert la façon dont la moyenne d'un échantillon se distribue. Il a pu dériver une formulation mathématique de cette distribution, la **distribution *t* de Student**, et à partir de là, nous verrons que de nombreuses applications en découlent. Nous pourrons, par exemple, dire si deux moyennes diffèrent **significativement** l'une de l'autre ou pas.

Mais au fait, pourquoi, cette distribution porte-t-elle le nom de "Student" ? Visionnez la vidéo suivante (malheureusement en anglais) pour le découvrir[^08-moyenne-1].

[^08-moyenne-1]: Vous pouvez activer les sous-titres en anglais à partir de la barre de boutons en bas de la vidéo pour vous aider à comprendre l'histoire.

```{r, echo=FALSE, purl=FALSE}
vembedr::embed_youtube("GFzCIA9kppM", width = 770, height = 433)
```

Le contrat que Gosset a signé avec son employeur l'empêchait de publier des résultats scientifiques sous son vrai nom. Ainsi, il décida de publier sa trouvaille qui occupe aujourd'hui une place très importante en statistiques sous le pseudonyme de "Student" (l'étudiant). Ce n'est qu'à sa mort, en 1937, que l'on put révéler le nom de l'auteur qui fut derrière cette fantastique découverte. Mais au fait, de quoi s'agit-il exactement ? Nous allons l'expliquer dans la section suivante.

## Distribution d'échantillonnage {#distributionechantillonnage}
...
\end{equation}
```
où $x$ est une variable quantitative (donc **numeric** dans R) et $n$ est la taille de l'échantillon, c'est à dire, le nombre d'individus mesurés. On notera $\bar{x}$ la moyenne de $x$, que l'on prononcera "x barre".
...
\end{equation}
```
À noter que $s^2$ est également appelée la **variance**[^08-moyenne-2].

[^08-moyenne-2]: L'équation proposée est, en fait, valable pour un échantillon et est calculée comme tel par R à l'aide des fonctions `sd()` pour l'écart type ou `var()` pour la variance. Pour la population ou pour un échantillon de taille très grande, voire infinie, nous pourrions plutôt diviser par $n$ au lieu de $n - 1$, ... mais puisque $n$ est très grand, cela ne change pas grand-chose au final.

En fait, ce qui nous intéresse, ce n'est pas vraiment la moyenne de l'échantillon, mais celle de la population que l'on notera $\mu$[^08-moyenne-3].

[^08-moyenne-3]: Notez que les lettres latines sont utilisées pour se référer aux variables et aux descripteurs statistiques telles que la moyenne pour l'échantillon, alors que les paramètres équivalents de la population, qui sont inconnus, sont représentés par des lettres grecques en statistiques.

##### À vous de jouer ! {.unnumbered}

`r h5p(86, "A08Ha_annotation", height = 400, toc = "Notation mathématique")`
...
La moyenne de la distribution d'échantillonnage est donc égale à la moyenne de la population et nous avons vu qu'elle peut être approximée par la moyenne d'un échantillon. Quant à l'écart type, il vaut 2/3 environ, soit l'écart type de la population divisé par 3.

##### À vous de jouer ! {.unnumbered}

`r h5p(87, "A08Hb_metaexperience", height = 400, toc = "Métaexpérience")`
...
```{=html}
<!-- Dans la SciViews Box, vous y accédez également via les "snippets" à partir de `.it` pour `(d)istribution: t (Student)` :

![](images/sdd1_09/t-distribution-snippets.png)
-->
```

```{r distristudent, echo=FALSE}
#'
...
Il s'agit du quantile 6,76. Tout cela est quand même un peu contraignant. Nous pouvons aussi utiliser la fonction `dist_student_t()` qui, elle, admet des arguments `df=`, `mu=` et `sigma=` afin d'effectuer les calculs pour n'importe quelle distribution *t* de Student, pas seulement la distribution réduite. Dans ce cas, les fonctions `cdf()` et `quantile()` ne permettent cependant pas de spécifier que l'on souhaite l'aire à droite, mais on se souvient que c'est égal à un moins l'aire à gauche.. Voilà ce que cela donne pour les deux calculs précédents :
...
L'utilisation de `dist_student_t()` simplifie considérablement les calculs... mais `<p|q|r|d>t()` restent plus concis si votre calcul concerne la distribution *t* de Student réduite. Le graphique correspondant au dernier calcul est le suivant :

```{r tdistri1, echo=FALSE, fig.cap="Une distribution de Student avec aire à gauche de 5% mise en évidence en rouge. La distribution Normale équivalente est superposée en bleu clair.", purl=FALSE}
dists <- c(Normal = dist_normal(mu = 8, sigma = 2/3), `Student t` = dist_student_t(df = 8, mu = 8, sigma = 2/3))
chart(dists) +
  geom_funfill(fun = dfun(student_t), from = -8, to = quantile(student_t, 0.05)) +
  geom_hline(yintercept = 0, col = "gray") +
  scale_color_manual(values = c("lightblue2", "black")) +
  ylab("Densité de probabilité") +
  annotate("text", x = 6.5, y = 0.02, label = "5%", col = "red")
```

Nous pouvons voir sur la Fig. \@ref(fig:tdistri1) que la distribution *t* de Student est plus resserrée en son centre, mais plus étalée aux extrémités que la distribution normale de même moyenne et écart type[^08-moyenne-5]. Néanmoins, elle est d'autant plus proche d'une normale que les degrés de liberté sont grands. On dit qu'elle converge vers une normale lorsque $ddl \longrightarrow \infty$. En pratique, pour des degrés de liberté égaux ou supérieurs à 30, nous pourrons considérer que les deux distributions sont pratiquement confondues.

[^08-moyenne-5]: Rappelez-vous que pour la distribution normale, le second paramètre est la variance et non l'écart type ce qui correspond à (2/3)\^2 = 0,44.

##### À vous de jouer ! {.unnumbered}

`r h5p(89, "A08Hc_loi_student", height = 400, toc = "Tendre vers la distribution normale")`
...
Vous avez remarqué quelque chose de particulier ? Oui, les deux aires à gauche et à droite sont identiques. C'est parce que la distribution est symétrique. On peut donc simplifier le calcul en récupérant l'aire d'un seul côté et en multipliant le résultat par deux :
...
```{r tdistri2, echo=FALSE, fig.cap="Une distribution de Student avec aire extrême de 5% mise en évidence en rouge.", purl=FALSE}
chart(student_t) +
  geom_funfill(fun = dfun(student_t), from = -8, to = quantile(student_t, 0.025)) +
  geom_funfill(fun = dfun(student_t), from = quantile(student_t, 1 - 0.025), to = 20) +
  geom_hline(yintercept = 0, col = "gray") +
  ylab("Densité de probabilité") +
...
Le dernier calcul que nous venons de faire (Fig. \@ref(fig:tdistri2)) correspond à ce que l'on nomme l'**intervalle de confiance à 95%** de la moyenne.
...
`r h5p(88, "A08Hd_confident", height = 400, toc = "Intervalle de confiance")`
...
```{r tdistri3, echo=FALSE, fig.cap="Une distribution de Student avec comparaison de l'IC 95% (entre les aires en rouge) et l'IC 90% (entre les aires en orange partiellement recouvertes par celles en rouge).", purl=FALSE}
chart(student_t) +
  geom_funfill(fun = dfun(student_t), from = -8, to = quantile(student_t, 0.05), fill = "orange") +
  geom_funfill(fun = dfun(student_t), from = quantile(student_t, 1 - 0.05), to = 20, fill = "orange") +
  geom_funfill(fun = dfun(student_t), from = -8, to = quantile(student_t, 0.025), fill = "red") +
  geom_funfill(fun = dfun(student_t), from = quantile(student_t, 1 - 0.025), to = 20, fill = "red") +
  geom_hline(yintercept = 0, col = "gray") +
  ylab("Densité de probabilité") +
  annotate("text", x = c(6.25, 9.75, 6.55, 9.45), y = 0.015,
    label = c("2.5%", "2.5%", " <5%", "5%>"), col = "red")
```

### Théorème central limite (encore)

Jusqu'ici, nous avons considéré une population au départ qui a une distribution normale, mais rien ne dit que ce soit le cas. Que se passe-t-il lorsque la distribution est différente ? Ici encore, nous pouvons effectuer une métaexpérience. Considérons, par exemple, une distribution uniforme de même moyenne = 8 et écart type = 2 que précédemment. Sachant que l'écart type d'une distribution uniforme vaut $\frac{max - min}{\sqrt{12}}$, voir [ici](https://fr.wikiversity.org/wiki/Variables_aléatoires_continues/Loi_uniforme), l'intervalle est de : $2 \cdot \sqrt{12} = 6,928$. Nous avons donc :
...
chart(data = NULL, ~ m_unif_n4) +
  geom_histogram(bins = 30) +
  geom_line(aes(x = x, y = dfun(normal1)(x) * 3000))
```
...
Partons d'un exemple concret. Imaginez que vous êtes des biologistes ouest-australiens travaillant à Fremantle. Vous y étudiez le crabe *Leptographus variegatus* (Fabricius, 1793). C'est un crabe qui peut se trouver en populations abondantes sur les côtes rocheuses fortement battues. Il a un régime alimentaire partiellement détritivore et partiellement carnivore.

![Crabe *Leptograpsus variegatus* par [Johnragla](https://commons.wikimedia.org/wiki/File:Purple_rock_crab_in_Aotea_Harbour_April_2013.jpg).](images/sdd1_08/Leptograpsus-variegatus.jpg)

Ce crabe est rapide et difficile à capturer... mais vous avez quand même réussi à en attraper et mesurer 200 d'entre eux, ce qui constitue un échantillon de taille raisonnable. Comme deux variétés co-existent, la variété bleue (`B`) et la variété orange (`O`) dans votre site d'étude, vous vous demandez si elles diffèrent d'un point de vue morphométrique. Naturellement, nous pouvons également supposer des différences entre mâles et femelles.

##### À vous de jouer ! {.unnumbered}

`r h5p(90, "A08He_echantillonnage", height = 400, toc = "Type d'échantillonnage")`
...
-   Les femelles ont-elle une carapace plus large ou moins large à l'arrière, en moyenne que les mâles ?
...
Appelons cette différence $\Delta rear$. Nous pouvons définir un intervalle de confiance pour $\Delta rear$ si nous pouvons calculer la valeur *t* ainsi que l'erreur standard $SE_{\Delta rear}$ associés à cette variable calculée. Après avoir interrogé des statisticiens chevronnés, ceux-ci nous proposent l'équation suivante pour $SE_{\Delta rear}$ (avec $n_F$ le nombre de femelles et $n_M$ le nombre de mâles) :

$$SE_{\Delta rear} = \sqrt{SE_{rear_F}^2 + SE_{rear_M}^2} = \sqrt{\frac{s_{rear_F}^2}{n_F} + \frac{s_{rear_M}^2}{n_M}}$$

Il nous reste à déterminer les degrés de liberté associés à la distribution *t*. Les statisticiens nous disent qu'il s'agit de *n* moins deux degrés de liberté. Nous obtenons alors l'équation suivante pour l'intervalle de confiance :
...
chart(student_t) +
  ...
  annotate("text", x = 3, y = 0.11, label = "Zones de rejet", col = "red") +
  annotate("segment", x = 2.5, y = 0.09, xend = 2.7, yend = 0.01,
    linewidth = 0.5, col = "red", arrow = arrow(length = unit(0.2, "cm"))) +
  annotate("segment", x = 2.5, y = 0.09, xend = -1.9, yend = 0.01,
    linewidth = 0.5, col = "red", arrow = arrow(length = unit(0.2, "cm"))) +
  annotate("text", x = 3.6, y = 0.3, label = "t[obs] == 4.29", parse = TRUE,
    col = "red", fontface = "bold.italic")
```

Nous pouvons calculer la valeur *p* nous-mêmes comme ceci, sachant la valeur de $t_{obs} = \frac{\Delta rear}{SE_{\Delta rear}}$ parce que nous travaillons avec une distribution *t* réduite :
...
Naturellement, R propose une fonction toute faite pour réaliser ce test afin que nous ne devions pas détailler les calculs à chaque fois. Il s'agit de la fonction `t.test()` dont les arguments sont les suivants :

-   le jeu de données dans `data =`

-   une formule qui reprend le nom de la variable quantitative à gauche (`rear`) et celui de la variable qualitative à deux niveaux à droite (`sex`)

-   l'indication du type d'hypothèse alternative, ici `alternative = "two-sided"` pour un test bilatéral

-   le niveau de confiance égal à $1 - \alpha$, donc `conf.level = 0.95`

-   enfin, si nous considérons les variances comme égales pour les deux sous-populations `var.equal = TRUE`.

<!-- Dans la SciViews Box, le snippet équivalent est accessible depuis `.hm` pour `hypothesis tests: means`. Dans le menu qui apparaît, vous choisissez `independant Student's t-test`. -->
...
Nous retrouvons exactement toutes les valeurs que nous avons calculées à la main **(donc, maintenant que vous connaissez la fonction `t.test()` vous ne vous amusez plus à faire tous les calculs précédents à la main... que nous avions réalisés ensemble seulement pour vous permettre de bien comprendre comment le test _t_ de Student est élaboré)**. Dans le cas présent, rappelez-vous la façon d'interpréter le test. Nous comparons la valeur *p* à $\alpha$. Si elle est plus petite, nous rejetons $H_0$, sinon, nous ne la rejetons pas. Ici, nous rejetons $H_0$ et pourrons dire que la largeur à l'arrière de la carapace de *L. variegatus* diffère de manière significative entre les mâles et les femelles au seuil $\alpha$ de 5% (test t bilatéral, *t* = 4,29, ddl = 198, valeur *p* \<\< 10^-3^).
...
`r learnr("A08La_ttest", title = "Moyenne et test t de Student", toc = "Moyenne et test t de Student")`

## Variantes du test *t* de Student {#studenttvariantes}
...
```{r ttest2, echo=FALSE, fig.cap="Visualisation de la distribution de Student réduite sous l'hypothèse nulle d'un test unilatéral à droite au seuil de 5%. Toute la zone de rejet est à droite.", purl=FALSE}
student_t <- dist_student_t(df = 198, mu = 0, sigma = 1)
chart(student_t) +
  geom_funfill(fun = dfun(student_t), from = quantile(student_t, 1 - 0.05), to = 5) +
  geom_hline(yintercept = 0, col = "gray") +
  xlim(-5, 5) +
  ylab("Densité de probabilité") +
...
Ce test sera nécessairement unilatéral à droite, car des concentrations négatives ne sont pas possibles. Dans d'autres circonstances, nous pourrions aussi avoir un test unilatéral à gauche en renversant l'inégalité pour l'hypothèse alternative et en plaçant toute la zone de rejet à la gauche de la distribution.
...
![Un test *t* indépendant nécessite une variable numérique et une variable facteur à deux niveaux.](images/sdd1_08/t-test-independent.png)
...
![Un test *t* apparié nécessite deux variables numériques mesurées sur les mêmes individus, donc les mêmes lignes dans un tableau cas par variables.](images/sdd1_08/t-tests-paired.png)
...
À chaque fois, les dimensions `front` et `rear` sont mesurées sur les mêmes individus. Nous soustrayons l'un de l'autre *d'abord* individu par individu.
...
La distribution de référence sous $H_0$ à partir de ces calculs est une $t(199, 0, 1)$, avec $n - 1$ comme degrés de liberté. Donc, la valeur *p* peut se calculer comme suit (n'oubliez pas de multiplier par deux pour reporter l'aire des deux côtés de la distribution pour un test bilatéral) :
...
Avant d'effectuer le test, pensez toujours à visualiser vos données avec la représentation la plus judicieuse. Ici, la boite de dispersion parallèle n'est pas très indiquée, car elle ne prend pas en compte l'appariement des données. Un graphique en nuage de points avec la bissectrice (pente de un et ordonnée à l'origine de zéro) ajoutée comme référence convient bien mieux. Sous $H_0$, le nuage de points est censé se concentrer sur cette bissectrice.
...
Le test correspondant s'obtient toujours avec la fonction `t.test()` mais cette fois-ci au lieu d'une formule, nous indiquons les deux variables suivant la notation `<data.frame>$<variable>`, et nous spécifions `paired = TRUE`. Fixons notre seuil $\alpha$ à 5% avant de faire le test, puis exécutons l'instruction suivante **(ici à nouveau, les détails de calculs ci-dessus ne sont plus nécessaire une fois que vous avez compris comment utiliser `t.test()` correctement pour un test apparié)** :

<!-- dans la SciViews Box à partir du menu `.hm` pour `hypothesis tests: means` et dans le menu qui apparaît, on sélectionne le "snippet" `paired Student's t test`. -->
...
Nous rejetons $H_0$, car la valeur *p* est inférieure à $\alpha$. Nous en concluons que le crabe *L. variegatus* a une carapace significativement plus large à l'avant qu'à l'arrière (t test apparié = 25,3, ddl = 199, valeur *p* \<\< 10^-3^). Naturellement, les variantes unilatérales à gauche et à droite existent aussi pour ce test. Par contre, la question des variances égales ou non ne se pose pas dans ce cas[^08-moyenne-7]. Notez que le test se limite à indiquer que la valeur *p* \< 2,2 . 10^-16^. Effectivement, notre calcul donne 3.7 . 10^-64^.

[^08-moyenne-7]: Pouvez-vous expliquer pourquoi ? Réfléchissez, c'est assez évident.

##### Conditions d'application {.unnumbered}

-   échantillon représentatif (échantillonnage aléatoire),
...
Comme exercice, interprétez ce test d'un point de vue statistique (rejet ou non de l'hypothèse nulle), et ensuite au niveau biologique (qu'est-ce que vous en concluez sur le phénomène biologique étudié) ?
...
![](images/sdd1_08/t-tests-univariate.png)
...
Le graphique suggère peut-être que les patient dorment plus, mais la boite de dispersion chevauche quand même le trait horizontal à zéro. Donc, qu'en est-il exactement ? Le test s'obtient toujours à partir de la fonction `t.test()`. Notez l'argument supplémentaire `mu =` qui permet de spécifier la valeur de référence (ici zéro). Fixons $\alpha$ avant d'effectuer le test. Ici, nous prenons 5%.

<!-- à partir du "snippet" `univariate Student's t-test` depuis `.hm` pour `hypothesis tests: means`.  -->
...
`r h5p(91, "A08Hf_variante_test_t", height = 400, toc = "Variantes du test t de Student")`

```{r assign_A08Ia_ttest, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("A08Ia_ttest", part = NULL,
    url = "https://github.com/BioDataScience-Course/A08Ia_ttest",
    course.ids = c(
      'S-BIOG-027' = !"A08Ia_{YY}M_ttest"),
    course.urls = c(
      'S-BIOG-027' = !"{assign_url$A08Ia_ttest}"),
    course.starts = c(
      'S-BIOG-027' = !"{class1_start(mod, 'A08')}"),
    course.ends = c(
      'S-BIOG-027' = !"{n3_end(mod, 'A08')}"),
    term = "Q2", level = 3,
    toc = "Tests de Student - adaptation d'algues")
```

## Test de Wilcoxon {#wilcox}

```{r testwilcoxon, echo=FALSE}
#'
...
<!-- accessible à partir des "snippets" dans le menu `.hn` pour `hypothesis tests: nonparametric` -->

Pour les données relatives à `med1` de `sleep2`, cela donne[^08-moyenne-8] :

[^08-moyenne-8]: Dans le cas d'un test de Wilcoxon apparié, on a $H_0: mediane_x = mediane_y$.
...
`r h5p(92, "A08Hg_test_student", height = 400, toc = "Choix du test d'hypothèse")`
...
![](images/sdd1_08/type_I_and_II_error.jpg)

L'erreur de première espèce (*type I error* en anglais) est associée au risque $\alpha$. L'erreur de seconde espèce (*type II error* en anglais) est associée au risque $\beta$. Plus $\beta$ est petit, plus le test est puissant. La puissance est $1 - \beta$. La puissance d'un test peut se calculer à l'aide des fonctions `pwr.xxx()` du package {pwr}. Par exemple dans le test *t* univarié concernant `med1` nous ferons :

```{r puissanceplot, echo=FALSE}
#'
...
![Le système judiciaire des statistiques, par Hadley Wickham.](images/sdd1_08/statistical-justice.jpg)

##### À vous de jouer ! {.unnumbered}

`r learnr("A08Lb_ttest_wmw", title = "Tests de Student et de Wilcoxon/Mann-Whitney", toc = "Tests de Student et de Wilcoxon/Mann-Whitney")`

## Représentation graphique {#representationgraphique}
...
Le graphique en barres (encore appelé graphique en dynamite, car il ressemble au déclencheur d'explosion de dynamite dans les dessins animés Walt Disney) et le nuage de points représentent identiquement la même information. Ils représentent la moyenne avec des barres d'erreurs qui matérialisent l'intervalle de confiance à 95%. Le graphique en dynamite avec des barres d'erreurs ne donne aucune information sur le nombre d'observations. Nous avons déjà abordé rapidement cette problématique dans la section \@ref(barres-mean). Il n'est donc pas le graphique optimal pour présenter un test *t* de Student. Donc, préférez systématiquement le graphique de droite ou une de ses variantes (graphique en violon au lieu des points et barres d'erreurs autour des moyennes superposées lorsque le nombre d'observations est très grand).

Malgré le fait que le test *t* de Student est un test paramétrique, on retrouve dans la littérature scientifique la boite à moustache qui est pourtant un graphique associé à des valeurs non paramétriques. Pour le test de Wilcoxon, la boite à moustache est le graphique descriptif le plus recommandé (on peut aussi y adjoindre les points, ou passer sur le graphique en violon si le nombre d'observations est très grand).
...
```{r, fig.cap="Nuage de points de la largeur de l'arrière de la carapace en focntion du sexe avec la moyenne et des barres d'erreurs. A : moyenne et intervalle de confiance 0.95. B : moyenne et écart-type. C : moyenne et 2*écart-type. D : moyenne et erreur standard"}
...
-   [Beware of dynamite](https://filedn.com/lzGVgfOGxb6mHFQcRn9ueUb/sdd-umons/sdd1_09/Koyama2003_BewareOfDynamite.pdf). Démonstration de l'impact d'un graphe en barres pour représenter la moyenne (et l'écart type) = graphique en "dynamite".
...
-   [Annotation de graphiques avec {{ggpubr}](https://delladata.fr/afficher-p-values-sur-graphiques/). Comment afficher les p-values sur un graph ? Ces variantes annotées des résultats des tests d'hypothèses correspondants se retrouvent fréquemment dans les publications en biologie.

<!-- Lien plus disponible ! [Comparaison de moyennes : indiquez la significativité des différences sur le graph](https://statistique-et-logiciel-r.com/comparaison-de-moyennes-indiquer-les-differences-significatives-sur-le-graph/). Tutoriel sur la comparaison de moyennes avec {ggpubr}. -->
...
```{r assign_A07Ga_human_II, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment2"))
  assignment2("A07Ga_human", part = "II",
    url = "https://github.com/BioDataScience-Course/A07Ga_human",
        course.ids = c(
      'S-BIOG-027' = !"A07Ga_{YY}M_human"),
    course.urls = c(
...
## Récapitulatif des exercices {#recapitulatifa08}

Vous venez de terminer le module 8. Ce module vous a permis d'apprendre une grande partie des subtilités qui se cachent derrière la moyenne qui peut nous sembler si simple. Il nous a aussi inité aux tests de Student et de Wilcoxon-Mann-Withney. Pour évaluer votre compréhension de cette matière, vous aviez les exercices suivants à réaliser :
...
-   [Podcast sections 8.2 - 8.6 (NotebookLM) - téléchargement](https://link.sciviews.org/A08_82-6podcast_notebooklm)
-   [Présentation vidéo sections 8.2 - 8.6 partiel (NotebookLM)](https://link.sciviews.org/A08_82-6video_notebooklm)
-   [Slides sections 8.2 - 8.6 (NotebookLM)](https://link.sciviews.org/A08_82-6slides_notebooklm)
-   [Infographics sections 8.2 - 8.6 partiel (NotebookLM)](https://link.sciviews.org/A08_82-6infographics_notebooklm)
-   [Quizz sections 8.2 - 8.6 (NotebookLM)](https://link.sciviews.org/A08_82-6quiz_notebooklm)

`r h5p(272, "A08Za_flash8", height = 400, toc = "Flashcards sections 8.2-8.6")`

##### Code des exemples {.unnumbered}

<!-- -  [Document HTML](more/08-Moyenne.html) -->
<!-- -  [Script R](https://github.com/BioDataScience-Course/sdd-umons-2025/blob/main/R/08-Moyenne.R){target="top"} -->

-  [Document HTML](https://link.sciviews.org/A08_25M_Rscript_html)
-  [Script R](https://link.sciviews.org/A08_25M_Rscript){target="top"}
