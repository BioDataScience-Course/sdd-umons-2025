...
Il est aussi possible de représenter *graphiquement* un tableau de contingence pour l'inclure dans une figure composée, éventuellement en le mélangeant à des graphiques[^05-donnees-qualitatives-2].

[^05-donnees-qualitatives-2]: Utilisez cette option avec parcimonie. Il vaut toujours mieux représenter un tableau comme ... un tableau plutôt que comme une figure !
...
Si vous insistez, vous pouvez aussi obtenir un tableau cas par variables (mais celui-ci est très long et peu pratique à manipuler) à l'aide de la fonction "speedy" `suncount()` (ou la fonction "tidy" `uncount()`)[^05-donnees-qualitatives-3] :

[^05-donnees-qualitatives-3]: Notez également que passer d'un tableau cas par variables à un tableau des fréquences d'occurrences se fait à l'aide de `scount()` ou `count()`.
...
Afin d'éviter que divers collaborateurs encodent différemment une information similaire, vous allez devoir préciser très clairement comment encoder les différentes variables de votre jeu de données. Par exemple pour une variable `genre`, est-ce que vous indiquez `homme` / `femme`, ou `h` / `f`, ou encore `H` / `F` ?
...
L'argument `cols =` est assez flexible. Toutes les formes ci-dessous permettent d'obtenir le même résultat (ici, nous passons sur la fonction tidy `pivot_longer()` équivalente).
...
[^05-donnees-qualitatives-7]: Notez malgré tout que, à condition de bien en comprendre les implications, le format complémentaire peut se justifier dans une publication pour y présenter un tableau le plus lisible possible, ce qui est le cas ici. Mais pour les analyses, c'est le format qui correspond à un tableau cas par variables qui **doit** être utilisé.
...
Parfois, ce sont les variables qui sont encodées de manière inappropriée par rapport aux analyses que vous souhaitez faire. Les fonctions speedy `sseparate()` et `sunite()`ou les fonctions tidy `separate()` et `unite()` permettent de séparer une colonne en plusieurs, ou inversement, de rassembler plusieurs colonnes en une seule.
...
```{r}
crabs2 <- sunite(crabs, col = "sp_sex", species, sex, sep = "_")
DT::datatable(crabs2)
```

Les fonctions complémentaires à `(s)unite()` sont `sseparate()` et `separate()`. Elles permettent de séparer une variable en deux ou plusieurs colonnes. Donc, pour retrouver un tableau similaire à celui d'origine, nous pourrons faire :

```{r}
crabs3 <- sseparate(crabs2, col = "sp_sex", into = c("species", "sex"), sep = "_")
DT::datatable(crabs3)
```
...
L'empilement des deux tableaux de données en un seul se fait en utilisant la fonction tidy `bind_rows()` ou la fonction speedy `sbind_rows()` lorsque les tableaux contiennent les mêmes variables présentées exactement dans le même ordre comme ici :
...
Observez bien ce dernier tableau. Les données initiales ont été *retriées* avant d'être fusionnées pour que les données correspondent. Comparez au tableau `nutrients` d'origine réimprimé ci-dessous :
...
`r learnr("A05Lb_multi_table", title = "Traitements multi-tableaux", toc = "Traitements multi-tableaux")`
...
**Effectuez également des remaniements de données multitableaux dans votre projet de groupe récurrent.**
