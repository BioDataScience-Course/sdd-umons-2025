...
Vous noterez que toutes le colonnes du tableau s'appellent "Allongement des dents". C'est le label de la variable `len`. Ce n'est pas très pratique. Vous pouvez changer ou créer un label explicite pour vos variables calculées en utilisant la fonction `labelise()`.

```{r}
tooth_summary <- summarise_(tooth,
  moyenne = ~fmean(len) |> labelise("Allongement moyen"),
  minimum = ~fmin(len) |> labelise("Allongement minimal"),
  médiane = ~fmedian(len) |> labelise("Allongement médian"),
  maximum = ~fmax(len) |> labelise("Allongement maximal"))
tabularise(tooth_summary)
```
...
```{r, warning=FALSE}
tooth_summary2 <- summarise_(group_by_(tooth, ~supp),
  moyenne = ~fmean(len) |> labelise("Allongement moyen"),
  minimum = ~fmin(len) |> labelise("Allongement minimal"),
  médiane = ~fmedian(len) |> labelise("Allongement médian"),
  maximum = ~fmax(len) |> labelise("Allongement maximal"),
  nobs    = ~fnobs(len) |> labelise("Nombre d'observations"))
tabularise(tooth_summary2)
```
...
```{r, warning=FALSE}
urchin_skeleton_summary <- summarise_(group_by_(urchin, ~origin),
  moyenne = ~fmean(skeleton, na.rm = TRUE),
  n       = ~fn(skeleton),    # Nombre total de cas
  nobs    = ~fnobs(skeleton)) # Nombre de cas hors valeurs manquantes
tabularise(urchin_skeleton_summary) |>
  Stb$set_header_labels(values = c(# Change le nom des colonnes
    "Origine", "Masse squelettique moyenne",
    "Observations totales", "Observations non manquantes"))
```
...
```{r}
tooth_summary4 <- summarise_(tooth,
